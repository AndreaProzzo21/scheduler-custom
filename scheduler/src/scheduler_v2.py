...
Rispetto alla versione base sono stati risolti i seguenti problemi:
  ...
  5. RESOURCE AWARENESS: CPU e RAM vengono SEMPRE controllate, anche quando il
     pod chiede una strategia diversa (latenza/temperatura/disk-io):
...
def calculate_best_node(api_instance, nodes_dict, annotations):
    ...
    Sceglie il nodo migliore in passi successivi:
      ...
      1. Filtro CPU/RAM soft: tra i candidati hardware-compatibili, esclude
         (con possibilità di override se TUTTI sono sovraccarichi) i nodi
         sopra soglia critica.
    ...
    if required_hw_tags:
        ...
        hw_matching_nodes = {
            n: ip for n, ip in candidate_nodes.items()
            if required_hw_tags.issubset(node_hw_tags.get(n, set()))
        }
        logging.info(
            f"🔧 Hardware richiesto {sorted(required_hw_tags)} -> nodi compatibili: "
            f"{sorted(hw_matching_nodes.keys()) if hw_matching_nodes else 'NESSUNO'}"
        )
        if not hw_matching_nodes:
            logging.error(
                f"❌ Nessun nodo soddisfa i requisiti hardware {sorted(required_hw_tags)}. "
                f"È un requisito rigido: il pod resta Pending, nessun fallback possibile."
            )
            return None
        candidate_nodes = hw_matching_nodes
    ...
    # --- 3. HARD FILTER CPU/RAM di sicurezza ---
    if resource_pct:
        safe_nodes = {
            n: ip for n, ip in candidate_nodes.items()
        ...
        if safe_nodes:
            excluded = set(candidate_nodes) - set(safe_nodes)
            if excluded:
                logging.info(
            ...
            logging.warning(
                "⚠️ TUTTI i nodi (compatibili con l'hardware richiesto) superano la soglia di sicurezza "
